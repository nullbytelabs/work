...
- **Agent steps** — `uses: agent/summarize` runs an agent **package** (a directory under `src/agents/<name>/`: `agent.yaml` manifest + `instructions.md` system prompt + `task.md` template) through an `AgentRunner` seam. The **default runner is the Pi coding-agent SDK** (`@earendil-works/pi-coding-agent`, optional dep, lazily loaded): it registers an OpenAI-compatible provider in-memory and drives `session.prompt()`, which resolves only after the full run **including retries** — the robustness a bare HTTP call lacks. (`OpenAiAgentRunner`, a dependency-free `fetch` call, remains a lighter fallback behind the same seam.) Providers/models come from a **config JSON** (`--config` / `pi-workflows.config.json`; `$ENV` apiKey expansion). `with:` binds the package's declared inputs into the task template; the result is `steps.<id>.outputs.summary`, and a `length` finish surfaces a truncation warning. The runner is injectable, so the whole pipeline is tested **without inference** (a mock runner).
...
│   ├── agent/       # AgentRunner seam + agent-package loader + OpenAI-compatible runner

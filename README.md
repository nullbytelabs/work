...
- **CLI** — two ways to launch: ad-hoc `./pi-workflows <workflow.yaml>` (run a file anywhere), or by name `./pi-workflows [--workspace <dir>] run <name>`, which finds the `.workflows/*.yaml` whose `name:` matches (workspace defaults to the current directory). Streams step output and exits non-zero on failure.

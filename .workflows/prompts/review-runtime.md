...
Review your scope in the workspace: open the source files under `src/runtime/`
and `src/targets/`, read them, trace callers and callees, and look for the most
important correctness, durability, or resource-handling issues — a run that
resumes wrong, a leaked VM, outputs threaded incorrectly, a race in the worker
loop. Read the actual code; don't guess.

...
> **This `ResourceLoader` surface is what the `work/agent` primitive lowers onto.** Pi has **no native single-name "agent" object** that bundles a system prompt + tools + model + extensions + skills — those are separate resource types assembled here. The engine's direction (see [`agent-primitive-and-actions.md`](agent-primitive-and-actions.md)) is a **dumb `work/agent` primitive** that maps its `with:`/supplied resources straight onto these Pi primitives — `instructions` → `systemPromptOverride`, `extensions` → `additionalExtensionPaths`, `skills` → `skillsOverride`, `tools` → `createAgentSession({ tools })` (pass-through, unpoliced), `model` → `modelRegistry.find(...)` — and lets a project's checked-in `.pi/` resources be **discovered ambiently** by the loader rooted at the checkout. A "bespoke agent" is then a user-space *action* that wraps `work/agent`, not an engine-owned package format. (The legacy `agent/<name>` handler still does its own **named** `{{ }}` input binding — Pi's file-based prompt templates take **positional** args (`$1`/`$@`, https://pi.dev/docs/latest/prompt-templates) — and calls `session.prompt(text, { expandPromptTemplates: false })`; under the reframe, templating moves out of the engine into the wrapping action.)

...
2. **Broker secrets through `work.json`.** A `secrets:` block whitelists which host
   secrets a workflow may address as `${{ secrets.NAME }}`. Each value is either
   plaintext or an `$ENV` reference (the same `$FIREWORKS_TOKEN` / `$GRAFANA_TOKEN`
   pattern already used for model `apiKey`). Whitelisted secrets flow through to the
   guest; anything not listed is unaddressable. This is the missing piece that makes
   `aws` / `gcloud` / `kubectl` usable (§3).

The non-negotiable we keep: **the model-key header-swap path is untouched.**
That is the mechanism that keeps the provider token out of agent calls, and it stays
...
target (`src/agent/egress.ts`):

| Job shape | Egress before | Where |
|---|---|---|
...
| `run:`-only | **fully denied** | `src/agent/egress.ts:57` returns `undefined` |
...
3. **The host-side header-swap is what isolates the model key.** The real value never
   enters the guest; Gondolin swaps a placeholder into the `Authorization` header
   for the model host and blocks it elsewhere (`src/targets/gondolin.ts`;
...
## 3. Why host-side header-swap can't cover the CLIs people want (VALIDATED)

The model-key path injects a credential as an **HTTP header**, host-side, so the real
value never enters the guest. Great for a bearer API (the model endpoint, or any
internal REST service you call the same way). Useless for the CLIs that drive real
infra work:
...
That is the `secrets:` passthrough (§4).
...
The **header-swap** broker stays exactly where the model key needs it: the per-host
model-key injection keeps the provider token *never in the guest* (path a). That is
the strong path for "call this host with a credential the workload never sees."
...
Keyed entries, value plaintext or `$ENV` reference:
...
The model key an **agent** uses still rides header-swap and never enters its guest —
unchanged, automatic, no operator action. The new `secrets:` passthrough is a
*separate*, operator-whitelisted set (your `aws` keys, a deploy PAT) meant chiefly
for `run:` steps.
...
operator explicitly hands it over. To keep a secret away from an agent step
entirely, split into two jobs — a `fetch` job holds the secret and exposes a job
output, and an `analyze` job (`needs: [fetch]`) consumes
`${{ needs.fetch.outputs.<name> }}` without the secret in scope. Referencing
`secrets.*` directly from an agent step is a supported, intentional choice when
in-guest is genuinely what you want.
...
- **The VM still isolates the host.** Filesystem, processes, kernel — the micro-VM's
  ...
  with the VM.
- **The strong path is still there, and still default for agents.** The model key you
  *don't* want in a guest goes through header-swap (path a), and a secret you want kept
  from an agent step rides a `fetch`/`analyze` job split. The `secrets:` passthrough is
  the explicit opt-in for the cases that genuinely need the value in-guest.
...
- **Slice C — ergonomics.** `work create secret <name>` merges a `secrets.<name>`
  entry via the config-merge writer (like `create webhook`); docs/templates show
...
remove; model-key injection to keep); `src/run.ts` (resolver wiring);
`src/targets/gondolin.ts`
